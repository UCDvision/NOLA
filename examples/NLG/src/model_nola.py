...
class QNOLA(nn.Module):
    # Quantized NOLA implementation of a dense layer
    def __init__(
        self, 
        in_features: int, 
        out_features: int, 
        ...
        **kwargs
        ...
        super(QNOLA, self).__init__(**kwargs)
        ...
        self.alpha = nn.Parameter(torch.ones(ka)*(1/self.ka), requires_grad=True)
        self.beta = nn.Parameter(torch.ones(kb)*(1/self.kb), requires_grad=True)
    ...
    def forward(self, x: torch.Tensor):
        ...
        c = self.scale_set[self.q_bits]/torch.max(torch.abs(self.alpha.detach()))
        alpha = self.alpha - self.alpha.detach() + torch.round((self.alpha.detach() * c))/c
        
        c = self.scale_set[self.q_bits]/torch.max(torch.abs(self.beta.detach()))
        beta = self.beta - self.beta.detach() + torch.round((self.beta.detach() * c))/c
        
        A = self.generate_params(alpha, 1, self.in_features, self.rank, self.seed_a) 
        B = self.generate_params(beta, 1, self.rank, self.out_features, self.seed_b) 
        
        return self.scale * ((x @ A) @ B)
    
    
class NOLA(nn.Module):
    # NOLA implementation of a dense layer
    def __init__(
        self, 
        in_features: int, 
        out_features: int, 
...
#         self.scale_A = 250.0 / self.ka
#         self.scale_B = 250.0 / self.kb
...
#         self.beta = nn.Parameter(torch.ones(kb), requires_grad=True)
    ...
    def forward(self, x: torch.Tensor):
        ...
        A = self.generate_params(self.alpha, 1, self.in_features, self.rank, self.seed_a) #* self.scale_A
        B = self.generate_params(self.beta, 1, self.rank, self.out_features, self.seed_b) #* self.scale_B
...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, apply_nola=False, qnola=False):
        ...
        self.qnola = qnola 
        self.apply_nola = apply_nola
        
        if apply_nola:
            if qnola: 
                self.nola_query = QNOLA(nx, n_state, ka=1000, kb=1000, rank=4, d=n_state, c=0.01)
                self.nola_value = QNOLA(nx, n_state, ka=1000, kb=1000, rank=4, d=n_state, c=0.01)
            else: 
                self.nola_query = NOLA(nx, n_state, ka=1000, kb=1000, rank=8, d=n_state, c=1.0)
                self.nola_value = NOLA(nx, n_state, ka=1000, kb=1000, rank=8, d=n_state, c=1.0)
    ...
    def prune(self, ratio=0.5): 
        self.pq.prune(ratio=ratio)
        self.pv.prune(ratio=ratio)
        
    def set_eval_mode(self,): 
        self.pq.set_eval_mode()
        self.pv.set_eval_mode()
    ...
    def forward(self, x, history=None, layer_past=None, len_past=None):
        ...
        if self.apply_nola:
            query = query + self.nola_query(hidden_states) 
            value = value + self.nola_value(hidden_states) 
...
class MLP(nn.Module):
    def __init__(self, n_state, config, apply_nola=False, qnola=False):  # in MLP: n_state=3072 (4 * n_embd)
        super(MLP, self).__init__()
        nx = config.n_embd
        
        if apply_nola: 
            if qnola: 
                self.p1_nola = QNOLA(n_state, nx, ka=1000, kb=1000, rank=4, d=nx, c=0.01)
                self.p2_nola = QNOLA(nx, n_state, ka=1000, kb=1000, rank=4, d=n_state, c=0.01)
            else:
                self.p1_nola = NOLA(n_state, nx, ka=1000, kb=1000, rank=8, d=nx, c=1.0)
                self.p2_nola = NOLA(nx, n_state, ka=1000, kb=1000, rank=8, d=n_state, c=1.0)
    ...
    def forward(self, x):
        if self.apply_nola: 
            h = self.act(self.c_fc(x) + self.p1_nola(x))
            h2 = self.c_proj(h) + self.p2_nola(h)
        else: 
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False, 
                 nola_mlp=False, nola_qv=False, 
                 nola_rank=8, nola_c=1.0, 
                 qnola=False, qbits=2):
        ...
        self.attn = Attention(nx, n_ctx, config, scale, 
                              apply_nola=nola_qv, nola_rank=nola_rank, nola_c=nola_c, qnola=qnola, qbits=qbits)
        self.ln_2 = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.mlp = MLP(4 * nx, config, 
                       apply_nola=nola_mlp, nola_rank=nola_rank, nola_c=nola_c, qnola=qnola, qbits=qbits)
...
class GPT2Model(nn.Module):
    def __init__(self, config):
        ...
        block = Block(config.n_ctx, config, scale=True, 
                      nola_mlp=config.nola_mlp, 
                      nola_qv=config.nola_qv,
                      nola_rank=config.nola_rank,
                      nola_c=config.nola_c,
                      qnola=config.qnola,
                      qbits=config.qbits)
...
class GPT2Config(object):
    def __init__(
        ...
        self.nola_rank=args.nola_rank 
        self.nola_qv=args.nola_qv 
        self.nola_mlp=args.nola_mlp 
        self.nola_c=args.c 
        self.qnola=args.qnola
...
class GPT2LMModel(nn.Module):
    ...
    def set_eval_mode(self):
        self.transformer.set_eval_mode()
