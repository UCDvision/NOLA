...
class LoraConfig(PeftConfig):
    ...
    use_nola: bool = field(
        default=False,
        metadata={"help": "Use NOLA method inside LoRA."},
    )
    nola_num_basis: int = field(default=128, metadata={"help": "Number of basis in NOLA."})
...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        kwargs = {
            "use_nola": lora_config.use_nola, 
            "nola_num_basis": lora_config.nola_num_basis, 
            "r": lora_config.r,
    ...
    def _find_and_replace(self, adapter_name):
        ...
        for key in key_list:
            ...
            if isinstance(target, LoraLayer) and isinstance(target, torch.nn.Conv2d):
                ...
                target.update_layer(
                    adapter_name,
                    lora_config.use_nola,
                    lora_config.nola_num_basis,
                    lora_config.r,
                    lora_config.lora_alpha,
                    lora_config.lora_dropout,
                    lora_config.init_lora_weights,
...
class LoraLayer:
    ...
    def update_layer(self, adapter_name, use_nola, nola_num_basis, r, lora_alpha, lora_dropout, init_lora_weights):
        self.r[adapter_name] = r
        self.lora_alpha[adapter_name] = lora_alpha
        self.use_nola[adapter_name] = use_nola
        self.nola_num_basis[adapter_name] = nola_num_basis
        ...
        if r > 0:
            if use_nola: 
                self.lora_A.update(nn.ModuleDict({adapter_name: 
                                                  NOLALinear(self.in_features, r, num_basis=nola_num_basis, bias=False)}))
                self.lora_B.update(nn.ModuleDict({adapter_name: 
                                                  NOLALinear(r, self.out_features, num_basis=nola_num_basis, bias=False)}))
            else:
    ...
    def reset_lora_parameters(self, adapter_name):
        ...
        if adapter_name in self.lora_A.keys():
            if self.use_nola[adapter_name]: 
                nn.init.zeros_(self.lora_A[adapter_name].coefficients)
                nn.init.zeros_(self.lora_B[adapter_name].coefficients)
...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.update_layer(adapter_name, use_nola, nola_num_basis, r, lora_alpha, lora_dropout, init_lora_weights)
