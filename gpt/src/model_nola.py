...
class GenerateParamsChunks(torch.autograd.Function):
    
    @staticmethod    
    def forward(ctx, coefficients, chunks, out_dim, in_dim, seed, scale): 

        num_basis = coefficients.shape[0]
        Out = torch.zeros(out_dim, in_dim).to(coefficients.device)
        chunks_length = num_basis//c 
        rand_seed = torch.randint(int(1e10), (1,))
        torch.manual_seed(seed)

        for i in range(chunks):  
            coefficients_ = coefficients[i*l:(i+1)*chunks_length]
            W_ = torch.zeros(chunks_length, out_dim, in_dim, device=coefficients.device)
            nn.init.uniform_(W_, a=-1.0, b=1.0)
            W_ = W_ * (scale/math.sqrt(num_basis)) 
            Out += torch.einsum('b,boi->oi', coefficients_, W_)
        
        params = torch.autograd.Variable(torch.tensor([chunks, out_dim, in_dim, seed, scale]))
    ...
    def backward(ctx, grad_output):
        ...
        chunks, out_dim, in_dim, seed, scale = params
        rand_seed = torch.randint(int(1e10), (1,))
        torch.manual_seed(seed)
        grad_coefficients = torch.empty(0).to(grad_output.device)
        chunks_length = torch.div(num_basis, chunks, rounding_mode='floor')
        ...
        for i in range(chunks_length):  
            ...
            W_ = W_ * (scale/math.sqrt(num_basis)) 
            W_ = W_.permute(1,2,0) 
            grad_coefficients = torch.cat((grad_coefficients, 
                                           torch.einsum('d,dl->l', grad_output.flatten(), 
                                                        W_.reshape(-1,chunks_length))) , dim=0) 

        torch.manual_seed(rand_seed)    
        return grad_coefficients , None, None, None, None, None
...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, use_lora=False, use_nola=False, 
                 nola_num_basis=1024, lora_rank=8, lora_alpha=1.0, qnola=False, qbits=2):
        ...
        if use_lora:  
            self.nola_query = LoRA(nx, n_state, use_nola=use_nola, 
                                   num_basis=nola_num_basis, rank=lora_rank, alpha=lora_alpha)
            self.nola_value = LoRA(nx, n_state, use_nola=use_nola,
                                   num_basis=nola_num_basis, rank=lora_rank, alpha=lora_alpha)
...
class MLP(nn.Module):
    def __init__(self, n_state, config, use_lora=False, use_nola=False, qnola=False, 
                 nola_num_basis=1024, lora_rank=8, lora_alpha=1.0, qbits=2):  # in MLP: n_state=3072 (4 * n_embd)
        super(MLP, self).__init__()
        nx = config.n_embd
        self.use_lora = use_lora 
        
        if use_lora: 
            self.p1_nola = LoRA(nx, n_state, use_nola=use_nola, 
                                num_basis=nola_num_basis, rank=lora_rank, alpha=lora_alpha)
            self.p2_nola = LoRA(n_state, nx, use_nola=use_nola, 
                                num_basis=nola_num_basis, rank=lora_rank, alpha=lora_alpha)
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False, 
                 lora_mlp=False, lora_qv=False, use_nola=False,
                 nola_num_basis=1024,
        ...
        self.attn = Attention(nx, n_ctx, config, scale, 
                              use_lora=lora_qv, use_nola=use_nola, 
                              nola_num_basis=nola_num_basis, 
                              lora_rank=lora_rank, lora_alpha=lora_alpha, 
                              qnola=qnola, qbits=qbits)
        self.ln_2 = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.mlp = MLP(4 * nx, config, 
                       use_lora=lora_mlp, use_nola=use_nola, 
                       nola_num_basis=nola_num_basis, 
                       lora_rank=lora_rank, lora_alpha=lora_alpha, 
                       qnola=qnola, qbits=qbits)
...
class GPT2Model(nn.Module):
    def __init__(self, config):
        ...
        block = Block(config.n_ctx, config, scale=True, 
                      lora_mlp=config.lora_mlp, 
                      lora_qv=config.lora_qv,
                      lora_rank=config.lora_rank,
                      use_nola=config.use_nola, 
                      nola_num_basis=config.nola_num_basis, 
